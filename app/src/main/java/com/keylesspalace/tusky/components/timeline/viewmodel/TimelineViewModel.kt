...
abstract class TimelineViewModel(
    ...
    fun init(kind: Kind, id: String?, tags: List<String>) {
        ...
        if (kind == Kind.HOME) {
            // Note the variable is "true if filter" but the underlying preference/settings text is
            // "true if show"
            filterRemoveReplies =
        ...
        viewModelScope.launch { eventHub.events.collect { event -> handleEvent(event) } }
    ...
    private fun reloadFilters() {
        viewModelScope.launch {
            api.getFilters()
                    .fold(
                            {
                                // After the filters are loaded we need to reload displayed content
                                // to apply them.
                                // It can happen during the usage or at startup, when we get
                                // statuses before filters.
                                invalidate()
                            },
                            { throwable ->
    ...
    enum class Kind {
        ...
        fun toFilterKind(): Filter.Kind {
            return when (valueOf(name)) {
                ...
                PUBLIC_FEDERATED, PUBLIC_LOCAL, TAG, FAVOURITES, PUBLIC_TRENDING_STATUSES ->
                        Filter.Kind.PUBLIC
