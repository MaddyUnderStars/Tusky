...
constructor(
        ...
        viewModelScope.launch {
            ...
            var detailedStatus =
                if (timelineStatus != null) {
                    ...
                    val viewData =
                        timelineStatus.toViewData(gson, isDetailed = true) as
                            StatusViewData.Concrete

                    // Return the correct status, depending on which one matched. If you do not
                    // do
                    // this the status IDs will be different between the status that's displayed
                    // with
                    // ThreadUiState.LoadingThread and ThreadUiState.Success, even though the
                    // apparent
                    // status content is the same. Then the status flickers as it is drawn
                    // twice.
                    ...
                    val result =
                        api.status(id).getOrElse { exception ->
    ...
    fun detailedStatus(): StatusViewData.Concrete? {
        return when (val uiState = _uiState.value) {
            is ThreadUiState.Success -> uiState.statusViewData.find { status -> status.isDetailed }
            is ThreadUiState.LoadingThread -> uiState.statusViewDatum
            else -> null
        }
    }

    fun reblog(reblog: Boolean, status: StatusViewData.Concrete): Job =
        viewModelScope.launch {
            try {
    ...
    fun translate(translate: Boolean, status: StatusViewData.Concrete): Job {
    ...
    fun favorite(favorite: Boolean, status: StatusViewData.Concrete): Job =
            viewModelScope.launch {
                try {
    ...
    fun bookmark(bookmark: Boolean, status: StatusViewData.Concrete): Job =
            viewModelScope.launch {
                try {
    ...
    fun voteInPoll(choices: List<Int>, status: StatusViewData.Concrete): Job =
            viewModelScope.launch {
                val poll =
                        status.status.actionableStatus.poll
                                ?: run {
                ...
                updateStatus(status.id) { status -> status.copy(poll = votedPoll) }
    ...
    fun removeStatus(statusToRemove: StatusViewData.Concrete) {
        updateSuccess { uiState ->
            uiState.copy(
                    statusViewData =
                            uiState.statusViewData.filterNot { status -> status == statusToRemove }
            )
        }
    }
    ...
    fun changeExpanded(expanded: Boolean, status: StatusViewData.Concrete) {
        updateSuccess { uiState ->
            val statuses =
                    uiState.statusViewData.map { viewData ->
            ...
            uiState.copy(statusViewData = statuses, revealButton = statuses.getRevealButtonState())
        }
    }

    fun changeContentShowing(isShowing: Boolean, status: StatusViewData.Concrete) {
        updateStatusViewData(status.id) { viewData -> viewData.copy(isShowingContent = isShowing) }
    }

    fun changeContentCollapsed(isCollapsed: Boolean, status: StatusViewData.Concrete) {
        updateStatusViewData(status.id) { viewData -> viewData.copy(isCollapsed = isCollapsed) }
    }
    ...
    private fun removeAllByAccountId(accountId: String) {
        updateSuccess { uiState ->
            uiState.copy(
                    statusViewData =
                            uiState.statusViewData.filter { viewData ->
    ...
    private fun handleStatusComposedEvent(event: StatusComposedEvent) {
        ...
        updateSuccess { uiState ->
            ...
            if (detailedIndex != -1 && repliedIndex >= detailedIndex) {
                ...
                val newStatuses =
                        statuses.subList(0, repliedIndex + 1) +
    ...
    private fun handleStatusDeletedEvent(event: StatusDeletedEvent) {
        updateSuccess { uiState ->
            uiState.copy(
                    statusViewData =
                            uiState.statusViewData.filter { status -> status.id != event.statusId }
            )
        }
    }
    ...
    private fun Status.toViewData(isDetailed: Boolean = false): StatusViewData.Concrete {
        val oldStatus =
                (_uiState.value as? ThreadUiState.Success)?.statusViewData?.find {
                    it.id == this.id
                }
        return toViewData(
    ...
    fun clearWarning(viewData: StatusViewData.Concrete) {
        updateStatus(viewData.id) { status -> status.copy(filtered = null) }
    }
